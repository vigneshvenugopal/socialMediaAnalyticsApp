...
namespace SocialMediaAnalytics
{
    public partial class fetchSocailMediaData : Form
    {
        ...
        private void Create_Param_Doc()
        {
            ...
            //var line_9 = string.Format("{0},{1}", "Hash Tag", twitterOfficialHandle);
            //csv.AppendLine(line_9);
            var line_9 = string.Format("{0},{1}", "Tweets From", twitterStartDate.Text == null ? "0" : DateTime.Parse(twitterStartDate.Text).ToString("yyyy-MM-dd"));
            csv.AppendLine(line_9);
            var line_10 = string.Format("{0},{1}", "Tweets Till", twitterEndDate.Text == null ? "0" : DateTime.Parse(twitterEndDate.Text).ToString("yyyy-MM-dd"));
            csv.AppendLine(line_10);
            var line_11 = string.Format("{0},{1}", "Tweet Count", 1000);
